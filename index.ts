...
async function main(): Promise<void> {
    ...
    const deploymentName = process.env.AZURE_OPENAI_DEPLOYMENT_NAME || 'gpt-realtime-2';
    ...
    realtimeClient.on('error', (receivedError: OpenAIRealtimeError) => receiveError(receivedError));
    realtimeClient.on('session.created', (receivedEvent: any) => receiveEvent(receivedEvent));
    realtimeClient.on('session.updated', (receivedEvent: any) => receiveEvent(receivedEvent));
    realtimeClient.on('response.output_audio.delta', (receivedEvent: any) => receiveEvent(receivedEvent));
    realtimeClient.on('response.output_audio_transcript.delta', (receivedEvent: any) => receiveEvent(receivedEvent));
    realtimeClient.on('response.done', (receivedEvent: any) => receiveEvent(receivedEvent));
