...
export class LiveSession {
  ...
  async start(config, history = [], { opening = false } = {}) {
    ...
    const c = {
      ...
      ready: false, closing: false, responses: new Map(), latestResponses: new Map(),
    ...
    try {
      ...
      tracks.forEach(track => {
        ...
        this.listen(c, track, 'mute', () => {
          if (!c.closing) this.onError('The browser paused microphone input. Keep this page open; reconnect if it does not resume.');
        });
      });
      ...
      c.readyTimer = this.later(c, () => this.fail(c, 'The voice session did not become ready. Reconnect to try again.'), 20000);
  ...
  send(c, event) {
    if (!this.isOpen(c) || !c.ready || c.channel?.readyState !== 'open') throw new Error('The voice connection is not ready.');
  ...
  receive(c, event) {
    ...
    if (event.type === 'session.closed') {
      this.finish(c, event.usage ?? null, true);
      return;
    }
    if (c.closing) return;
    if (event.type === 'session.started') {
      ...
      c.stream.getAudioTracks().forEach(track => { track.enabled = !this.muted; });
      ...
      if (c.ready) this.onTranscript(event);
      ...
      if (!c.ready) this.fail(c, message);
      else {
  ...
  setOutputMuted(muted) {
    ...
    if (!muted) void this.resumePlayback(c);
  }
  ...
  checkHealth(online = navigator.onLine !== false) {
    ...
    if (state === 'failed' || state === 'closed') {
      ...
      else if (!c.ready) this.setState('connecting');
    }
  }
  ...
  stop({ immediate = false } = {}) {
    ...
    if (c.ready && c.channel?.readyState === 'open') {
    ...
    if (c.ready) this.finish(c);
    else this.cleanup(c, 'ended');
    return c.closed;
  }

  finish(c, usage = null, confirmed = false) {
    if (!this.isCurrent(c)) return;
    this.onClosed({ usage, sessionId: c.sessionId, confirmed });
  ...
  fail(c, message) {
    if (!this.isCurrent(c)) return;
    this.onError(message);
    if (c.ready && !c.closing && c.channel?.readyState === 'open') {
