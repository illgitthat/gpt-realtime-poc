  ...
  append(event) {
    ...
    if (!message) {
      ...
      const position = this.messages.findIndex(existing => existing.transport === this.transport &&
        start !== null && existing.start !== null &&
        (existing.start > start || (existing.start === start && role === 'user' && existing.role === 'assistant')));
      if (position === -1) this.messages.push(message);
      else this.messages.splice(position, 0, message);
    }
