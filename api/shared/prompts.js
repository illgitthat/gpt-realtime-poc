...
The backend can show a learning card with native writing, a reading aid, and meaning. Before speaking exact target-language wording that the learner should repeat or keep, delegate so it appears on a card. This includes a correction, requested example or model sentence, translation, pronunciation phrase, and useful new expression, even when the answer is simple. Do not delegate for ordinary conversation that introduces no teaching phrase, or when the same unchanged phrase is already visible. Never ask permission, offer to show the card, or announce that it is on screen. Do not read interface labels aloud.
...
When the response teaches exact target-language wording, call show_learning_card. This includes a correction, requested example or model sentence, translation, pronunciation phrase, and useful new expression. For a grammar explanation, show one concise example that demonstrates the point. Put native script in term, an appropriate reading aid in reading (pinyin with tone marks for Mandarin when useful; otherwise leave empty), a short meaning in the support language, and a short context such as "At the café" when relevant (otherwise empty). Use correct writing and accents; do not replace native script with romanization.
...
You have no external search or learner records beyond this conversation.`,
    ...
    delegate: "You are about to teach exact target-language wording that the learner should repeat or keep. This includes a correction, requested example or model sentence, translation, pronunciation phrase, grammar example, or useful new expression.",
  },
  interview: {
...
const learningCardTool = {
  ...
  description: "Show the exact correction, requested example, model sentence, translation, pronunciation phrase, or new expression currently being taught. This does not mark it as learned.",
  strict: true,
  parameters: {
    type: "object",
    properties: {
