...
function validatePayload(body) {
  ...
  const voice = text(body.voice, "voice", 30, "marin");
  if (!VOICES.has(voice)) throw new RequestError("Unknown assistant voice.");
  ...
  return { sdp, mode, voice, instructions, settings, input };
}
...
async function createLiveSession({ payload, env, signal }) {
  const { sdp, mode, voice, instructions, settings, input } = validatePayload(payload);
  ...
  const session = {
    ...
    delegation: {
      type: "responses",
      responses: {
        model: env.AZURE_OPENAI_REASONING_DEPLOYMENT_NAME?.trim() || "gpt-6-luna",
  ...
  // Finish before the browser's 25-second connection deadline, including on SWA
  // where the legacy Functions adapter cannot forward a client disconnect signal.
  const timeout = AbortSignal.timeout(24000);
  let response;
  try {
    // Creating a session is billable and not idempotent. Never retry this POST invisibly.
    response = await fetch(`${baseUrl}/live/sessions`, {
      method: "POST",
