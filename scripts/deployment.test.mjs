...
test("runtime environment contains only four fields with the shared defaults", () => {
  const result = readRuntimeEnv(values);
  assert.deepEqual(Object.keys(result), runtimeEnvNames);
  assert.equal(result.AZURE_OPENAI_BASE_URL, "https://gateway.example.com/general/openai/v1");
  assert.equal(result.AZURE_OPENAI_DEPLOYMENT_NAME, "gpt-live-1");
  assert.equal(result.AZURE_OPENAI_REASONING_DEPLOYMENT_NAME, "gpt-5.6-luna");
  assert.deepEqual(readDeploymentEnv(JSON.stringify(values), "LIVE_PREVIEW_ENV"), result);
  assert.deepEqual(readDeploymentEnv(JSON.stringify(values), "LIVE_PRODUCTION_ENV"), result);
  assert.equal(readRuntimeEnv({ ...values, AZURE_OPENAI_DEPLOYMENT_NAME: "custom-live" }).AZURE_OPENAI_DEPLOYMENT_NAME, "custom-live");
});
...
test("SWA CLI and hosted Functions use the same Node runtime", () => {
  const cli = JSON.parse(readFileSync(new URL("../swa-cli.config.json", import.meta.url), "utf8"));
  const hosted = JSON.parse(readFileSync(new URL("../public/staticwebapp.config.json", import.meta.url), "utf8"));
  const app = cli.configurations["gpt-realtime-poc"];
  assert.equal(`${app.apiLanguage}:${app.apiVersion}`, hosted.platform.apiRuntime);
  assert.equal(app.apiLocation, "api");
  assert.equal(app.appLocation, "public");
  assert.deepEqual(hosted.routes, [{ route: "/connect", rewrite: "/api/connect" }]);
});
...
test("preview and production deploy code and secrets atomically with stdin-only runtime credentials", () => {
  for (const [deploy, options, target, keep] of [
    ...
    [deployProduction, productionOptions, "", ["--keep-vars"]],
  ]) {
    const calls = [];
    ...
    assert.equal(args[3], 'cat | exec "$@"');
    assert.deepEqual(args.slice(7), ["deploy", "--env", target, ...keep, "--secrets-file", "/dev/stdin"]);
