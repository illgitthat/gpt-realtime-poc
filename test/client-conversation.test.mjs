...
import { Conversation, recentHistory, languageTag, runDisplayTool, validateLearningCard } from '../public/conversation.js';
...
test('captions preserve Spanish punctuation, split words, whitespace, and repeated words exactly', () => {
  ...
  assert.equal(conversation.messages[0].fragments.length, 6);
});
...
const card = { language: 'Chinese', term: '你好', reading: 'nǐ hǎo', meaning: 'Hello' };
...
test('display tool returns success only after rendering validated native-script text', () => {
  ...
  assert.equal(languageTag('<img src=x onerror=alert(1)>'), '');
});
...
test('display tool rejects unknown names, wrong modes, invalid JSON and field types or lengths', () => {
  ...
  assert.equal(validateLearningCard({ ...card, reading: '', meaning: '' }).card.term, '你好');
});
