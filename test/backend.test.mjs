...
test("Live negotiation uses APIM once, keeps credentials server-side, and restores native-script history", async () => {
  const requests = [];
...
test("deployment names are configurable independently and tutor exposes only its display tool", async () => {
  let session;
  await createLiveSession({
    env: { ...env, AZURE_OPENAI_DEPLOYMENT_NAME: "voice-deployment", AZURE_OPENAI_REASONING_DEPLOYMENT_NAME: "reasoning-deployment" },
    payload: { sdp, mode: "tutor", settings: { language: "Chinese", level: "Beginner" } },
  ...
  assert.match(session.delegation.responses.instructions, /pinyin/);
});

test("invalid requests fail before any billable request", async () => {
  const invalid = [
    null, [], { sdp: "" }, { sdp, mode: "gemini" }, { sdp, instructions: 42 },
  ...
  for (const payload of invalid) {
    await assert.rejects(createLiveSession({
      payload, env,
      fetchImpl: () => assert.fail("Invalid input must not contact Azure"),
    }), { name: "RequestError", status: 400 });
  }
});

test("a rejected or interrupted billable POST is not retried and upstream data is not exposed", async () => {
  for (const status of [400, 401, 403, 429, 500]) {
    ...
    await assert.rejects(createLiveSession({
      ...
      fetchImpl: async () => {
        calls++;
        return Response.json({ error: { message: "private-upstream-data" } }, { status });
      },
    }), error => {
...
test("malformed service responses do not masquerade as a connection", async () => {
  for (const data of [{}, { session: { id: "x" }, transport: { sdp } }, { ...answer, transport: { type: "webrtc", sdp: "" } }]) {
    await assert.rejects(createLiveSession({
      payload: { sdp }, env, fetchImpl: async () => Response.json(data),
